...
import gcp_implementation.ga_colorize_original_adaptative as ga_colorize


def plot_fitness_over_generations(best_fitness_per_generation, output_dir, experiment_name, graph_name):
    plt.figure()
    ...
    plt.title(f'Best Fitness per Generation Across Runs\nExperiment: {experiment_name}, Graph: {graph_name}')
...
def run_experiment(graph_path, ga_params_path):
    ...
    first_4_color_generation = []
    first_min_color_generation = []
    colors_per_generation = []
    ...
    for run_index in range(ga_params['num_runs']):
        ...
        population_size = ga_params['num_individuals']
        ...
        for generation in range(ga_params['num_generations']):
            ...
            diversity_run.append(compute_genetic_diversity(individuals))
        ...
        diversity_per_generation.append(diversity_run)
    ...
    return (generation_times, total_times, fitness_values, best_fitness_per_generation,
            run_results, stopping_generations, first_4_color_generation, first_min_color_generation,
            colors_per_generation, diversity_per_generation)


def plot_diversity(diversity_per_generation, output_dir, experiment_name, graph_name):
    plt.figure()
    for i, run in enumerate(diversity_per_generation):
        plt.plot(range(len(run)), run, alpha=0.7, linewidth=2, label=f'Run {i + 1}')
    plt.xlabel('Generation')
    plt.ylabel('Genetic Diversity')
    plt.title(f'Genetic Diversity per Generation Across Runs\nExperiment: {experiment_name}, Graph: {graph_name}')
    plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, 'genetic_diversity_per_generation.png'), bbox_inches='tight')
    plt.close()


def compute_genetic_diversity(individuals):
    """
    Compute genetic diversity of a population by measuring the average Hamming distance
    between individuals.
    """

    def hamming_distance(bin_seq1, bin_seq2):
        return sum(c1 != c2 for c1, c2 in zip(bin_seq1, bin_seq2))

    # Convert individuals to binary strings
    bin_individuals = [format(individual, f'0{len(bin(individuals[0])) - 2}b') for individual in individuals]

    # Calculate pairwise Hamming distances
    distances = [hamming_distance(ind1, ind2) for ind1, ind2 in combinations(bin_individuals, 2)]
        ...
        for file in os.listdir(folder):
            if file.endswith("experiment_results.csv"):
                ...
                if df.empty:
                    ...
                    columns = ['Experiment', 'Graph', 'Run', 'Avg Generation Time', 'Total Run Time',
                               'Final Fitness Value', 'Best Fitness Value', 'Avg Number of Colors Used',
                               'Average Total Time', 'Average Fitness', 'Stopping Generation',
                               'First 4 Color Generation', 'First Min Color Generation']
...
def run_all_experiments(ga_params_folder, graphs_folder, results_folder):
    ...
    for ga_params_file in os.listdir(ga_params_folder):
        ...
        for graph_file in os.listdir(graphs_folder):
            ...
            (generation_times, total_times, fitness_values, best_fitness_per_generation, run_results,
             stopping_generations, first_4_color_generation, first_min_color_generation, colors_per_generation,
             diversity_per_generation) = run_experiment(
                graph_path, ga_params_path)
            plot_results(generation_times, total_times, fitness_values, best_fitness_per_generation, output_dir,
                         experiment_name, graph_name)
            plot_diversity(diversity_per_generation, output_dir, experiment_name, graph_name)
            plot_fitness_over_generations(best_fitness_per_generation, output_dir, experiment_name, graph_name)
            ...
            save_results_csv(csv_filename, generation_times, total_times, fitness_values, avg_total_time, avg_fitness,
                             ...
                             best_fitness_per_generation, first_4_color_generation, first_min_color_generation,
                             colors_per_generation)
